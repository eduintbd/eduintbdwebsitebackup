      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-2xl font-bold">{feedbackData.fluencyCoherence}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Lexical Resource</p>
            <p className="text-2xl font-bold">{feedbackData.lexicalResource}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Grammar Range & Accuracy</p>
            <p className="text-2xl font-bold">{feedbackData.grammaticalRange}</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground">Pronunciation</p>
            <p className="text-2xl font-bold">{feedbackData.pronunciation}</p>
          </div>
        </div>
        <div className="pt-4 border-t">
          <h4 className="font-semibold mb-2">Detailed Feedback:</h4>
          <p className="text-sm whitespace-pre-wrap">{feedbackData.feedback}</p>
        </div>
      </CardContent>
    </Card>
  );
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Part {part}: {part === 1 ? "Interview" : part === 2 ? "Long Turn" : "Discussion"}</CardTitle>
            <Button onClick={() => generateTopic(part)} disabled={isGenerating}>
              ...
              <div className="p-4 bg-secondary/20 rounded-lg space-y-2">
                ...
                <div className="space-y-1">
                  {topic.questions.map((q, idx) => (
                    <p key={idx} className="text-sm">• {q}</p>
                  ))}
                </div>
